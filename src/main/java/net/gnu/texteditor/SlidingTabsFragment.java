...
public class SlidingTabsFragment extends Fragment implements TabAction {
	...
	public void addTab(Object...objs) {//}, final String charset, final boolean changed, final int linebreak) {
		final Intent intent = (Intent) objs[0];
		final String title = (String) objs[1];
	...
	static class PagerItem implements Parcelable {
		 ...
		 */
		Main createFrag(Intent intent, String title) {
			Log.d(TAG, "createFrag() " + title + ", " + frag);
			if (frag == null) {
				frag = Main.newInstance(intent, title);
			}
			return frag;
		}
		...
		Main getFrag() {
			...
			if (frag == null) {
				frag = Main.newInstance(null, title);
			}
			return frag;
		}
